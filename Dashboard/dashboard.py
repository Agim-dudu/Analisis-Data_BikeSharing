...
# Prepare dataframe
df = pd.read_csv("C:\\Users\\agimd\\Program Data Saints\\DBS Foundation\\Mentor\\Analisis Data\\Dashboard\\data_clean.csv")
...
season_palette = sns.color_palette("Set2", 7)
user_palette = sns.color_palette("Set2", 7)

fig, ax = plt.subplots(figsize=(10, 6))
sns.barplot(x="season", y="jumlahPengguna", hue="tipePengguna", data=createSeason, palette=user_palette)
...
casual_patch = mpatches.Patch(color=user_palette[0], label='Casual')
registered_patch = mpatches.Patch(color=user_palette[1], label='Registered')
...
sns.barplot(x="casual", y="season", data=sum_casual_user_df, palette=season_palette, ax=ax[0])
...
sns.barplot(x="registered", y="season", data=sum_registered_user_df, palette=season_palette, ax=ax[1])
...
sns.barplot(y="cnt", x="weathersit", data=byweather, palette=season_palette, ax=ax)
...
sns.barplot(y="recency", x="weekday", data=rfm_df.sort_values(by="recency", ascending=True).head(5), palette=season_palette, ax=ax[0])
ax[0].set_ylabel(None)
ax[0].set_xlabel(None)
ax[0].set_title("By Recency (days)", loc="center", fontsize=18)
ax[0].tick_params(axis ='x', labelsize=15)

sns.barplot(y="frequency", x="weekday", data=rfm_df.sort_values(by="frequency", ascending=False).head(5), palette=season_palette, ax=ax[1])
ax[1].set_ylabel(None)
ax[1].set_xlabel(None)
ax[1].set_title("By Frequency", loc="center", fontsize=18)
ax[1].tick_params(axis='x', labelsize=15)

sns.barplot(y="monetary", x="weekday", data=rfm_df.sort_values(by="monetary", ascending=False).head(5), palette=season_palette, ax=ax[2])
...
st.caption('Copyright © M. Gymnastiar 2024')
